...
class TestBezier(unittest.TestCase):
    def test_power_conversion(self):
        ...
        pg[1, 0] = 1
        pg[0, 1] = 1
        Pg = bezier_lyapunov.power_to_bernstein_poly(pg)
        ...
        pf[0, 1] = 1
        pf[2, 1] = 1
        Pf = bezier_lyapunov.power_to_bernstein_poly(pf)
    ...
    def test_bernstein_mul(self):
        ...
        p1_bern = bezier_lyapunov.power_to_bernstein_poly(p1)
        p2_bern = bezier_lyapunov.power_to_bernstein_poly(p2)
        power_mul_bern = bezier_lyapunov.power_to_bernstein_poly(np.flip(power_mul))

        mul = bezier_lyapunov.bernstein_mul(p1_bern, p2_bern)
    ...
    def test_bernstein_add(self):
        ...
        p1_bern = bezier_lyapunov.power_to_bernstein_poly(p1)
        p2_bern = bezier_lyapunov.power_to_bernstein_poly(p2)
        power_add_bern = bezier_lyapunov.power_to_bernstein_poly(np.flip(power_add))

        add = bezier_lyapunov.bernstein_add(p1_bern, p2_bern)
        np.testing.assert_allclose(add, power_add_bern)

    def test_bernstein_derivative(self):
        p = np.array([9, 5, 1])
        p_der = np.polyder(np.flip(p))
        p_bern = bezier_lyapunov.power_to_bernstein_poly(p)
        p_der_bern = bezier_lyapunov.power_to_bernstein_poly(np.flip(p_der))

        der = bezier_lyapunov.bernstein_derivative(p_bern)
    ...
    def test_bernstein_integral(self):
        ...
        integral = bezier_lyapunov.bernstein_integral(p_bern)
