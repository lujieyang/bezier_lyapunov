...
import mcint
from utils import extract_polynomial_coeff_dict, matrix_adjoint, matrix_det, reconstruct_polynomial_from_dict
import pickle
...
def acrobot_lqr_ROA():
    ...
    prog.AddSosConstraint((z-z0).dot(z-z0)*(V - rho) - lam*V_dot)
...
if __name__ == '__main__':
    ...
    # prog = MathematicalProgram()
    # z = prog.NewIndeterminates(6, "z")
    # with open("acrobot/data/sos/J_iterative_4_upper_bound_deg_{}.pkl".format(deg), "rb") as input_file:
    #     C = pickle.load(input_file)
    # J_star = reconstruct_polynomial_from_dict(C, z)
    # dz = 0.03
    # params_dict = acrobot_setup()
    # params_dict["x_max"] = np.array([np.pi+dz, dz, 1, 1])
    # params_dict["x_min"] = np.array([np.pi-dz, -dz,  -1, -1])
    # plot_value_function(Polynomial(J_star), z, params_dict, deg, file_name="saturation/level_set")
