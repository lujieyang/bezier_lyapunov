...
def planar_pusher_sos_lower_bound(deg, objective="integrate_ring", visualize=False, test=False):
    ...
    d_theta = np.pi/3
    z_max = np.array([0.3, 0.3, np.sin(d_theta), 1, px])
    z_min = np.array([-0.3, -0.3, -np.sin(d_theta), np.cos(d_theta), -px])
    ...
    lam_d = prog.NewSosPolynomial(Variables(z), int(np.ceil(lam_deg/2)*2))[0].ToExpression()
...
def planar_pusher_single_contact_switching_sos_lower_bound(deg, objective="integrate_ring", eps=1e-3, visualize=False, test=False):
    """x = [x, y, theta, px, py]
    z = [x, y, s, c, px, py]
    u = [fn, ft, v_px, v_py]
    """
    ...
    lam_c = prog.NewFreePolynomial(Variables(z), lam_deg).ToExpression()
    lam_d = prog.NewFreePolynomial(Variables(z), lam_deg).ToExpression()
    S_complementarity = -lam_a*v_py*ft + lam_b*(ft**2 - mu_p**2*fn**2) + lam_c*(ft**2 - mu_p**2*fn**2)*v_py -lam_d * v_px * v_py
    lam_a = prog.NewSosPolynomial(Variables(z), int(np.ceil(lam_deg/2)*2))[0].ToExpression()
    lam_b = prog.NewFreePolynomial(Variables(z), lam_deg).ToExpression()
    lam_c = prog.NewFreePolynomial(Variables(z), lam_deg).ToExpression()
    lam_d = prog.NewFreePolynomial(Variables(z), lam_deg).ToExpression()
    S_complementarity += -lam_a*v_px*ft + lam_b*(ft**2 - mu_p**2*fn**2)*v_px -lam_c * v_px * n[0] - lam_d * v_py * n[1]

    S_n = 0
    vs = [np.array([px, px]), np.array([-px, px]), np.array([px, -px]), np.array([-px, -px])]
    for v in vs:
    ...
    for i in range(nz):
        lam = prog.NewSosPolynomial(Variables(z), int(np.ceil(lam_deg/2)*2))[0].ToExpression()
        S_Jdot += lam*(z[i]-z_max[i])*(z[i]-z_min[i])
    prog.AddSosConstraint(LHS + S_ring + S_Jdot + S_complementarity + S_s + S_n)
    ...
    S_n = 0
    for v in vs:
        lam_v = prog.NewSosPolynomial(Variables(z), deg)[0].ToExpression()  
        S_n += lam_v * (-px - v.dot(n))     
    # n0 * n1 =0
    ...
    os.makedirs("planar_pusher/data/single", exist_ok=True)
    os.makedirs("planar_pusher/figures/single", exist_ok=True)

    save_polynomial(J_star, z, 'planar_pusher/data/single/J_lower_deg_{}_100.pkl'.format(deg))
    if visualize:
        plot_value_function(J_star, z, z_max, x2z, file_name="single/lower_bound_constrained_lqr_{}_{}_100".format(objective, deg), plot_states="xtheta", switch_contact=True)
    return J_star, z
